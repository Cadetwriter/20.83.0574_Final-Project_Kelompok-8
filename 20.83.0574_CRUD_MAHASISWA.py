...
while lanjutkan:
    ...
    if(pilihan == 1):
        ...
        alamat = input("Alamat: ")
        sql = "INSERT INTO tblmahasiswa (nomor, NIM, nama, jenis_kelamin, jurusan, alamat VALUES (%s, %s, %s, %s, %s, %s)"
        ...
        dbcursor.execute("SELECT * FROM tblmahasiswa where id ="+id+" LIMIT 1")
        ...
        if(mahasiswa != None):
            ...
            dbcursor.execute(sql, val)
            con.commit()
            print(dbcursor.rowcount, "data user berhasil di simpan")
        else:
        ...
        if(mahasiswa != None):
            ...
            dbcursor.execute(sql)
            con.commit()
            print(dbcursor.rowcount, "data user berhasil di hapus")
        else:
            print("data tidak ditemukan")
    elif(pilihan == 5):
        lanjut = False